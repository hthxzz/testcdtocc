...
<template>
  <div class="vehicles-operate-query-container">
    <BaseLayoutContent>
      <template #leftPanel>
        <panel>
          ...
          <PanelItem title="车辆排名[辆]" :displaySwitch="false" class="rangePanel">
    ...
    <ptus
      style=" position: absolute;top: 140px;left: 40%;width:365px;"
      v-if="isshowhide"
      :test="vehicleNo"
...
<script>
import ptus from "./components/ptus1.vue";
...
import RealtimeBroadcast from "./components/realtimeBroadcast1.vue";
import EnterpriseVehicleQuery from "./components/enterpriseVehiclesQuery1.vue";
  ...
  methods: {
    ...
    getBaseInfoHtml(type, data, feaCoordinate, titleText, width, height) {
      ...
      if (type.indexOf("AREA_VEHICLES") !== -1) {
        this.vehicleNo = data.vehiclePlateNum;
        this.alarmTypeName = data.alarmTypeName;
        this.alarmTime = data.alarmTime;
        if (this.isshowhide == true) {
          this.$refs.ptus.getVehicle(data.vehiclePlateNum,data.alarmTypeName,data.alarmTime);
        }
        ...
        this.alarmTime = data.alarmTime;
        if (this.isshowhide == true) {
          this.$refs.ptus.getVehicle(data.vehicleNo,data.alarmTypeName,data.alarmTime);
        }
        this.isshowhide = true;
        return null;
