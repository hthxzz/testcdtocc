              ...
              <div class="OperationComplaint-wrapper">
                ...
                >
                  ...
                  >
                    ...
                    <el-table-column label="准班率" prop="quasiRate"></el-table-column>
                    <!-- <el-table-column label="统计方式" prop="hour"></el-table-column> -->
                    <el-table-column label="起点站名称" prop="station"></el-table-column>
                    <el-table-column label="班次" prop="departureNum"></el-table-column>
                    <el-table-column label="上座率" prop="attendanceRate"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getStationPassengerFlow(val1, val2) {
      this.$http
        .$get("kyjc_transportStatistic/getStationDepartureInfoByStation", {
          cycleType: val1 || "year",
        ...
        .then(res => {
          ...
          for (const item of res) {
            xAxisData.push(item.station);
            departureNum.push(item.departureNum);
            // lastFlowNum.push(item.lastFlowNum);
          }
          seriesData.push(departureNum);
          // seriesData.push(lastFlowNum);
          this.option1 = this.$utils.echart.getBarOption(
            {
              xAxisData: xAxisData,
              legendData: ["班次", "去年同期客流量"],
    ...
    getDatePassengerFlow(val1, val2) {
      this.$http
        .$get("kyjc_transportStatistic/getStationDepartureInfoByDate", {
          cycleType: val1 || "year",
          date: val2 || this.$utils.dayjs().year()
        })
        .then(res => {
    ...
    getPassengerFlowByPage(val) {
      ...
      this.$http
        .$get(
          "kyjc_transportStatistic/getStationDepartureInfoByDateAndStation",
          val
        )
        .then(res => {
          // this.getSearchTableValue();
          ...
          for (const item of res && res) {
            xAxisData.push(item.date);
            departureNum.push(item.departureNum);
            attendanceRate.push(item.attendanceRate);
          }
          seriesData.push(departureNum);
          seriesData.push(attendanceRate);
  ...
  data() {
    return {
      ...
      url: "/kyjc_transportStatistic/getStationDepartureInfoByPage",
      option1: {},
      option2: {},
      option3: {},
      stationName: "",
