...
export default {
  ...
  methods: {
    ...
    getDatePassengerFlow(val1, val2) {
      this.$http
        .$get("kyjc_statisticHandler/getPassengerFlowByDate", {
          cycleType: val1 || "year",
          date: val2 || this.$utils.dayjs().year()
        })
        .then(res => {
    ...
    getPassengerFlowByPage(val) {
      // debugger;
      this.$http
        .$get("kyjc_statisticHandler/getPassengerFlowByDateAndStation", val)
        .then(res => {
          // this.getSearchTableValue();
          const xAxisData = [];
          const seriesData = [];
  ...
  data() {
    return {
      ...
      url: "/kyjc_statisticHandler/getPassengerFlowByPage",
      option1: {},
      option2: {},
      option3: {},
      stationName: "",
