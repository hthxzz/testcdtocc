...
export default {
  ...
  methods: {
    ...
    handleInfo(index, row) {
      this.$http
        .$get("/gxdc_bikeAreaHandler/getAreaInfoById", { id: row.id })
        .then(data => {
      ...
      // 消除上一个图层
      tmap.removePolygonOnLayer("SHAREBIKE_FOCUS_AREA", this.beginId);
    },
    // 修改  重绘
    handleEdit(index, row) {
      // 消除上一个图层
      try {
        tmap.removePolygonOnLayer("SHAREBIKE_FOCUS_AREA", row.id);
      } catch {}
    ...
    update(data) {
      if (data) {
        ...
        this.$http
          .$post("/gxdc_bikeAreaHandler/updateAreaInfo", {
    ...
    handleDelete(index, row) {
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        this.$http
          .$get("/gxdc_bikeAreaHandler/deleteAreaInfoById", { id: row.id })
          .then(data => {
            if (data === 1) {
              this.$message({
                type: "success",
    ...
    getData(data) {
      if (data) {
        ...
        this.$http
          .$post("gxdc_bikeAreaHandler/AddAreaInfo", {
            data: JSON.stringify(params)
          })
          .then(data => {
            if (data === 1) {
  ...
  data() {
    return {
      url: "/gxdc_bikeAreaHandler/getAreaInfoByPage",
      areaName: "",
      areaData: "",
      updateId: "",
      updateAreaName: "",
